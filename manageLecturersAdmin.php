...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update') {
        ...
        if (!empty($newPhone) && !preg_match('/^01[0-9]{8}$/', $newPhone)) {
            echo json_encode(['success' => false, 'error' => 'Phone number must start with 01 and be exactly 10 digits long.']);
            exit;
        }
            ...
            checkedBoxes.forEach((cb, index) => {
                ...
                const blockHtml = `
                    <div class="bulk-lecturer-block" data-index="${index}">
                        ...
                        <div class="form-group">
                        <label>Phone</label>
                        <input type="text" class="bulk-phone" value="${phone}" required placeholder="e.g. 0123456789" maxlength="10" pattern="01[0-9]{8}" title="Phone number must start with '01' followed by exactly 8 digits" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                        </div>
        ...
        function saveEditedLecturer(e) {
            ...
            if (bulkBlocks.length > 0) {
                ...
                const promises = totalPayloads.map(payload => {
                    ...
                    return fetch('manageLecturersAdmin.php', {
                        method: 'POST',
                        body: formData
                    }).then(res => res.json());
                });
