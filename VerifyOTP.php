...
if ($conn->connect_error) { die("Sambungan database gagal."); }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['resend_otp'])) {
        ...
        try {
            ...
            $mail->send();
            $message = "<div style='color: #10b981; margin-bottom: 15px;'>Kod OTP baharu berjaya dihantar ke emel anda!</div>";

        } catch (Exception $e) {
            $message = "<div style='color: #ef4444; margin-bottom: 15px;'>Gagal menghantar semula emel: {$mail->ErrorInfo}</div>";
        }
    }
}
$conn->close();
