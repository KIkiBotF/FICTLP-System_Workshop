...
");
if (!$dropdown_stmt) {
    die("SQL Prepare Error (Dropdown): " . $conn->error);
}
...
if ($selected_subject !== '') {
    ...
    // Get Total Passes
    // FIXED: Joined chapter table to check Subject_Code
    $pass_stmt = $conn->prepare("
        ...
        WHERE c.Subject_Code = ? AND s.Grade >= q.Passing_Mark
    ");
    if (!$pass_stmt) {
        die("SQL Prepare Error (Passes): " . $conn->error);
    }
    ...
    ");
    if (!$fail_stmt) {
        die("SQL Prepare Error (Fails): " . $conn->error);
    }
    ...
    $details_stmt = $conn->prepare("
        ...
        JOIN quiz q ON s.Quiz_ID = q.Quiz_ID
        JOIN chapter c ON q.Chapter_ID = c.Chapter_ID
        WHERE e.Subject_Code = ? AND c.Subject_Code = ?
            ...
            <div class="report-container">
                <form method="GET" action="reportLecturer.php" class="filter-row">
                    ...
                    <select id="subjectSelect" name="subjectSelect" onchange="this.form.submit()">
                        ...
                        <?php
                        // Loop through the subjects this lecturer teaches
